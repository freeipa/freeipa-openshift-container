...
@test "container_step_clean_directories" {
    skip "FIXME 'mock output' does not work properly"

    source './init/tasks.inc.sh'
    source './init/container.inc.sh'

    function helper_container_step_clean_directories
    {
        container_step_clean_directories
    }
    export -f helper_container_step_clean_directories

    mock stub rm container_helper_clean_directories_print_out
    mock_container_helper_clean_directories_print_out 0
    mock stdout mock_container_helper_clean_directories_print_out <<EOF
/run/secrets
/run/systemd/notify
/tmp/var/tmp/test
/tmp/test
EOF
    ...
    run helper_container_step_clean_directories
    assert_success
    assert_mock rm

    mock unstub rm container_helper_clean_directories_print_out
}
...
@test "container_step_do_check_terminate_await - args==no-exit" {
    ...
    export ARGS=("no-exit")
    unset DEBUG_NO_EXIT
    mock stub mkdir container_helper_write_no_poweroff_conf tasks_helper_shift_args
    ...
    assert_success
    assert_mock mkdir
    assert_mock container_helper_write_no_poweroff_conf
    assert_mock tasks_helper_shift_args
    mock unstub mkdir container_helper_write_no_poweroff_conf tasks_helper_shift_args
}
...
@test "container_step_do_check_terminate_await - args[0]==exit-on-finished" {
    ...
    mock stub mkdir container_helper_link_to_power_off tasks_helper_shift_args
    mock_mkdir 0 -p /run/systemd/system/ipa-server-configure-first.service.d
    ...
    assert_success
    assert_mock mkdir
    assert_mock container_helper_link_to_power_off
    assert_mock tasks_helper_shift_args
    mock unstub mkdir container_helper_link_to_power_off tasks_helper_shift_args
}
...
@test "container_step_read_command - ipa-replica-install" {
    ...
    function helper_container_step_read_command {
        container_step_read_command
        assert_equal "${COMMAND}" "ipa-replica-install"
    }
    export -f helper_container_step_read_command
...
@test "container_step_process_hostname" {
    skip "TODO It is not implemented"
}
...
@test "container_step_save_ipa_server_ip_if_provided" {
    skip "TODO It is not implemented"
}
